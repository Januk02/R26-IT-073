...
const CVVerification = ({ onComplete, onCancel }) => {
  ...
  const updateMentorVerificationStatus = async (cvScore) => {
    const interviewSnapshot = await getDocs(
      query(collection(db, "verifications"), where("mentorId", "==", user.uid)),
    );
    const bestInterview = interviewSnapshot.docs
    ...
    const combinedScore = bestInterview
      ? bestInterview.overallScore * 0.6 + cvScore * 0.4
      : null;
    const status =
      combinedScore === null
        ? "pending-interview"
    ...
    await setDoc(
      doc(db, "users", user.uid),
      {
        ...
        mentorCombinedScore:
          combinedScore === null ? null : Math.round(combinedScore),
  ...
  useEffect(() => {
    ...
    try {
      ...
      replaceLatestCVVerification(latestPendingVerification)
        ...
        .catch((error) =>
          console.error("Error retrying CV verification save:", error),
        );
    } catch (error) {
  ...
  const extractTextFromPDF = async (arrayBuffer) => {
    try {
      ...
      console.log(
        `PDF extraction complete. Extracted ${fullText.length} characters`,
      );
      ...
      if (fullText.length < 100) {
        ...
        throw new Error(
          "Extracted text is too short - PDF might be image-based",
        );
      }
      ...
      try {
        ...
        if (hasReadableText) {
          console.log(
            `Alternative extraction successful: ${text.length} characters`,
          );
          return text;
        } else {
        ...
        throw new Error(
          "Unable to extract text from PDF. The PDF might be image-based, scanned, or password-protected.",
        );
      }
    }
  };
  ...
  const extractCVProfile = (cvText, analysis) => {
    ...
    const experienceMatches =
      normalizedText.match(/\b(\d+)\+?\s*(years?|yrs?)\b/g) || [];
    const experienceYears = experienceMatches.reduce(
      (highest, match) => Math.max(highest, Number(match.match(/\d+/)[0])),
      0,
    );
    const profileFields =
      fields.length > 0 ? fields : ["General Professional Profile"];
  ...
  const analyzeCV = async (cvText) => {
    ...
    console.log(
      "Unique Words:",
      [...new Set(text.match(/\b\w{4,}\b/g) || [])].length,
    );
    ...
    const mentorScore = calculateMentoringScore(
      text,
      criteria.mentoringSpecific,
    );
    ...
    const evidenceGate = calculateEvidenceGate(
      text,
      criteria,
      totalScore,
      cvType,
    ...
    const { feedback, strengths, recommendations } =
      generateProfessionalFeedback(criteria, totalScore, cvType);
    ...
    return {
      ...
      criteria: Object.keys(criteria).map((key) => ({
        ...
        percentage: Math.round(
          (criteria[key].score / criteria[key].weight) * 100,
        ),
  ...
  const calculateEducationScore = (text, criteria) => {
    ...
    const advancedFound = criteria.advanced.filter((keyword) =>
    ...
    const standardFound = criteria.standard.filter((keyword) =>
      text.includes(keyword),
    ).length;
    if (standardFound > 0) {
    ...
    const basicFound = criteria.basic.filter((keyword) =>
      text.includes(keyword),
    ).length;
    if (basicFound > 0) {
    ...
    const certFound = criteria.certifications.filter((keyword) =>
      text.includes(keyword),
    ).length;
    if (certFound > 0) {
    ...
    const institutionsFound = criteria.institutions.filter((keyword) =>
      text.includes(keyword),
    ).length;
  ...
  const calculateExperienceScore = (text, criteria) => {
    ...
    const seniorityFound = criteria.seniority.filter((keyword) =>
      text.includes(keyword),
    ).length;
    if (seniorityFound > 0) {
    ...
    const leadershipFound = criteria.leadership.filter((keyword) =>
      text.includes(keyword),
    ).length;
    ...
    const mentoringFound = criteria.mentoring.filter((keyword) =>
      text.includes(keyword),
    ).length;
    if (mentoringFound > 0) {
    ...
    if (
      text.includes("10–15 hours") ||
      text.includes("10-15 hours") ||
      text.includes("10 to 15 hours")
    ) {
      score += 5;
  ...
  const calculateMentoringScore = (text, criteria) => {
    ...
    if (
      text.includes("mentoring experience") ||
      text.includes("mentoring approach")
    ) {
      score += 3;
    ...
    if (
      text.includes("helped junior students") ||
      text.includes("junior students")
    ) {
      score += 2;
  ...
  const detectCVType = (text) => {
    ...
    const mentorCount = mentorIndicators.filter((indicator) =>
      text.includes(indicator),
    ).length;
    const qaCount = qaIndicators.filter((indicator) =>
      text.includes(indicator),
    ).length;
    const devCount = devIndicators.filter((indicator) =>
      text.includes(indicator),
    ).length;
  ...
  const calculateEvidenceGate = (text, criteria, score, cvType) => {
    ...
    const hasQuantifiedEvidence =
      /\b\d+\+?\s*(years?|mentees?|students?|participants?|engineers?|interns?|users?)\b/.test(
        text,
      );
  ...
  const generateProfessionalFeedback = (criteria, totalScore) => {
    ...
    if (criteria.education.score >= 15) {
      ...
      recommendations.push(
        "Consider highlighting advanced degrees or certifications",
      );
    }
    ...
    if (criteria.experience.score >= 20) {
      ...
      recommendations.push(
        "Add more detail about years of experience and seniority level",
      );
    }
    ...
    if (criteria.mentoringSpecific.score >= 10) {
      ...
      recommendations.push(
        "Include specific mentoring experiences and outcomes",
      );
    }
    ...
    if (totalScore >= 85) {
      ...
      feedback.push(
        "Profile needs significant development for mentorship role",
      );
    }
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (!cvText || cvText.length < 50) {
        setError(
          "Could not extract sufficient text from the PDF file. Please ensure the PDF contains readable text.",
        );
      ...
      const verificationData = {
        ...
        verificationStatus:
          result.overallScore >= 50 ? "qualified" : "needs-retry",
        verificationStatusLabel:
          result.overallScore >= 50 ? "Qualified" : "Needs Re-attempt",
        cvProfile,
      };
      const pendingKey = `pendingCvVerification:${user.uid}`;
      localStorage.setItem(pendingKey, JSON.stringify([verificationData]));

      replaceLatestCVVerification(verificationData)
        .then(() => {
          localStorage.removeItem(pendingKey);
        ...
        .catch((saveError) => {
          ...
          setError(
            "Analysis completed, but the verification could not be saved. Please refresh and try again.",
          );
        });
      ...
      setDoc(
        ...
        console.error(
          "Error saving CV profile to mentor document:",
          profileError,
        );
      });
    } catch (err) {
        ...
        <div className="max-w-3xl w-full bg-white rounded-2xl shadow-xl p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600">
              Your CV has been analyzed for mentorship suitability
            </p>
            <div className="mt-3">
              <span className="inline-block px-4 py-1.5 bg-gradient-to-r from-blue-100 to-blue-200 text-blue-800 rounded-full text-sm font-semibold">
                🏆{" "}
                {getMedalInfo(getMedalTier(analysisResult.overallScore)).name}{" "}
                Earned!
              </span>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-r from-purple-50 to-purple-100 rounded-xl p-6 mb-6 border border-purple-200">
            <div className="text-center">
              ...
              <div className="flex items-center justify-center gap-3">
                <p
                  className={`text-5xl font-bold ${getScoreColor(analysisResult.overallScore, 100).split(" ")[0]}`}
                >
                  {Math.round(analysisResult.overallScore)}%
                </p>
                <span
                  className={`px-3 py-1 rounded-full text-sm font-medium ${getScoreColor(analysisResult.overallScore, 100)}`}
                >
            ...
            {analysisResult.criteria.map((criterion, index) => (
              <div key={index} className="bg-gray-50 rounded-lg p-4">
                ...
                <p className="text-xs text-gray-500 mt-1">
                  {Math.round(criterion.percentage)}% match
                </p>
              </div>
            ))}
          </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-purple-50 flex items-center justify-center px-4 py-8">
      <div className="max-w-2xl w-full bg-white rounded-2xl shadow-xl p-8">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-600">
            Upload your CV for AI-powered analysis to assess your mentorship
            qualifications
          </p>
        </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="font-semibold text-green-900">
                  You already submitted a CV
                </p>
                <p className="mt-1 text-sm text-green-700">
                  {existingCV.fileName || "Previous CV"}
                  {existingCV.uploadedAt &&
                    ` • ${new Date(existingCV.uploadedAt).toLocaleDateString()}`}
                </p>
              </div>
            </div>
            <button
        ...
        <div
          ...
          onDrop={handleDrop}
          onClick={() =>
            (!existingCV || isResubmitting) && fileInputRef.current?.click()
          }
        ...
        >
          ...
          {file ? (
            ...
            <div className="flex flex-col items-center">
              ...
              <p className="text-sm text-gray-500">
                Choose “Resubmit CV” above to replace it
              </p>
            </div>
          ) : (
            <div className="flex flex-col items-center">
              ...
              <>
                ...
                >
                  <path
                    ...
                    strokeLinejoin="round"
                    strokeWidth={2}
                    d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6 4h8a2 2 0 002-2v-8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"
                  />
                </svg>
                Analyze CV
              </>
