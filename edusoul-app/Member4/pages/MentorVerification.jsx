...
const QUESTIONS = [
  ...
  },
  {
    id: 6,
    domain: "Expectations",
    questions: ["What do you expect from an alumni?"],
  },
  {
    id: 7,
    domain: "Adaptability",
    questions: ["How would you adjust your approach to different students?"],
  },
  {
    id: 8,
  ...
  },
  {
    ...
    questions: ["What would you do if a alumni disagreed with your advice?"],
  },
];
...
const MentorVerification = ({ onComplete, onCancel }) => {
  ...
  const analyzeAnswer = async (question, answerText) => {
    ...
    const profTermsFound = professionalTerms.filter((term) =>
      text.includes(term),
    ).length;
    ...
    if (
      question.domain === "Handling Challenges" &&
    ...
    if (
      question.domain === "Communication & Relationship" &&
      text.includes("trust") &&
  ...
  const getVerificationStatus = (score) =>
    score >= 50 ? "qualified" : "needs-retry";
  const getVerificationStatusLabel = (score) =>
    score >= 50 ? "Qualified" : "Needs Re-attempt";
  ...
  const updateMentorVerificationStatus = async (interviewScore) => {
    ...
    const currentInterviewScore = bestInterview
      ? bestInterview.overallScore
      : interviewScore;
    const combinedScore = latestCV
      ? currentInterviewScore * 0.6 + latestCV.overallScore * 0.4
      : null;
    const status =
      combinedScore === null
        ? "pending-cv"
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      // AI scoring runs only on the backend so credentials remain private.
      try {
        const questionsPayload = allQuestions.map((q) => ({
          question: q.question,
          domain: q.domain,
        }));
        const result = await mentorshipApi.analyzeInterview(
          questionsPayload,
          answers,
        );
        ...
        console.error("AI interview scoring failed:", backendErr.message);
        alert(
          `AI interview scoring could not be completed: ${backendErr.message}`,
        );
        return;
      }
          ...
          {isAnalyzing ? (
            ...
            <>
              <div className="text-center mb-8">
                ...
                <p className="text-gray-600">
                  Your mentorship interview has been analyzed
                </p>
                <div className="mt-3">
              ...
              <div className="bg-purple-50 rounded-xl p-6 mb-6 border border-purple-200">
                <div className="text-center">
                  ...
                  <p className="text-xs text-gray-400 mt-2">
                    Scores are automatically generated by AI analysis
                  </p>
                </div>
              </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-purple-50 flex items-center justify-center px-3 py-4 sm:px-6">
      <div className="max-w-6xl w-full bg-white rounded-2xl shadow-xl p-4 sm:p-6 lg:p-8 pb-24">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="text-sm font-bold text-gray-900">
                AI Avatar Interview Host
              </h4>
              <p className="text-xs text-gray-600">
                Free Interactive Avatar asks questions & answers clarifications
              </p>
            </div>
          </div>
              ...
              <div className="flex items-center gap-4 mb-4">
                ...
                {recordings[currentQuestionIndex] && (
                  <>
                    <button
                      onClick={() =>
                        playRecording(recordings[currentQuestionIndex])
                      }
                      ...
                      onClick={() => {
                        ...
                        setTimeout(() => {
                          const mockTranscription =
                            "I've been mentoring students for over 5 years...";
        ...
        {(isSubmitting || isAnalyzing) && (
          <div className="mb-5">
            <div className="flex items-center justify-between text-sm text-purple-700 font-medium mb-2">
              <span>Submitting your interview…</span>
              <span>{isSubmitting ? "Processing" : "Analyzing"}</span>
            </div>
            <div className="w-full bg-purple-100 rounded-full h-2.5 overflow-hidden">
              <div
                className="h-full rounded-full bg-gradient-to-r from-purple-600 to-purple-400 animate-pulse"
                style={{ width: "72%" }}
              />
            </div>
          </div>
        ...
        <div className="sticky bottom-0 z-20 mt-3 border-t border-gray-200 bg-white/95 px-3 py-3 shadow-[0_-4px_16px_rgba(0,0,0,0.08)] backdrop-blur sm:static sm:border-0 sm:bg-transparent sm:p-0 sm:shadow-none">
          <div className="mx-auto flex max-w-6xl justify-between gap-3 sm:max-w-none">
            <button
              ...
              disabled={
                currentQuestionIndex === 0 || isSubmitting || isAnalyzing
              }
            ...
            <button
              ...
              className="px-6 py-3 rounded-lg font-semibold bg-purple-600 text-white hover:bg-purple-700 transition-colors disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2 min-w-[220px]"
            >
              {isSubmitting || isAnalyzing ? (
                <>
