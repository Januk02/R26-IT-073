...
async def get_recommendations(student_profile: StudentProfile):
    ...
    try:
        print(f"Received request data: {student_profile}")
        
        # Initialize the backward-chaining model
        model = BackwardChainingModel()
        
        # Get recommendations
        results = model.backward_chain(student_profile.dream_job, student_profile.dict())
        
        print(f"Generated recommendations: {results}")
        if "error" in results:
        ...
        counterfactual = generate_counterfactual_guidance(student_profile.dict(), results)
...
async def predict_simple(student_data: dict):
    """Simple prediction endpoint for testing"""
    try:
        dream_job = student_data.get("dream_job", "Software Engineer")
        z_score = student_data.get("z_score", 1.5)
        stream = student_data.get("stream", "Physical Science")
        
        # Simple logic for demonstration
        if dream_job == "Software Engineer" and stream in ["Physical Science", "Mathematics"]:
            degree = "IT"
            confidence = min(0.9, z_score / 1.5)
        elif dream_job == "Doctor" and stream == "Bio Science":
            degree = "Medicine"
            confidence = min(0.9, z_score / 2.0)
        elif dream_job == "Data Scientist" and stream in ["Physical Science", "Mathematics"]:
            degree = "IT"
            confidence = min(0.85, z_score / 1.4)
        else:
            degree = "Business"
            confidence = min(0.7, z_score / 1.2)
