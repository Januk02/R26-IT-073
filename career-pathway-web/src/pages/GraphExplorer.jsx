...
/* ─────────────────────────────────────────────
   CONSTANTS — colors & icons for each group
   ───────────────────────────────────────────── */
const PALETTE = {
...
/* ─────────────────────────────────────────────
   LAYOUT — Left-to-Right Transition Pipeline
   Maps coordinates exactly to 0-100% space.
   ───────────────────────────────────────────── */
...
/* ─────────────────────────────────────────────
   ...
   Using 0-100 viewBox to align 1:1 with CSS percentages
   ───────────────────────────────────────────── */
      ...
      {links.map((l, i) => {
        ...
        return (
          <path key={i}
            ...
            fill="none"
            stroke={`url(#${gid})`}
            strokeWidth={lit ? 0.6 : 0.25} // coordinates are 0-100, so thin stroke width works best
...
export default function GraphExplorer() {
  ...
  const handleDoubleClick = useCallback((e) => {
    // Prevent toggling if clicked on specific interactive controls (like panel, dropdown, buttons, etc.)
    if (
      e.target.closest('.gn') || 
    ...
    setIsMovable(prev => {
      const next = !prev;
      if (!next) {
        setScale(1); // Reset scale when disabling free move
      }
      return next;
    });
  }, []);
  ...
  // Auto-select first pathway if none is active
  useEffect(() => {
